    ...
    <body>-->
        <div class="row">
            <div class="col-12">
				...
				<table width="100%">
					<tr>
						<td valign="top" width="35%" style="padding-left:0px;">
									...
									<?php 
										...
										if(!empty($laborAttendanceList)): 
											foreach($laborAttendanceList as $row):
												if($row->type == 2):
													$logData = json_decode($row->log_data);
													...
													if (!empty($logData)) :
														foreach($logData as $log):
															if($log->present > 0):
																echo '<tr>
																	<td>'.$i++.'</td>
																	<td>'.ucfirst($log->labor_category).'</td>
																	<td>'.floatval($log->present).'</td>
																</tr>';
																$total += floatval($log->present);
															endif;
														endforeach;
													endif;
												else:
													...
													foreach($logData as $key=>$val):
														...
														if ($key == 'male' || $key == 'female') {
															$key = 'Unskilled Labour'; 
														}
														if (!isset($laborArr[$key])) {
															$laborArr[$key] = 0;
														}
														$laborArr[$key] +=floatval($val);
													endforeach;
												endif;
											endforeach;
											foreach($laborArr as $key=>$val):
												echo '<tr>
													<td>'.$i++.'</td>
													<td>'.ucfirst($key).'</td>
													<td>'.floatval($val).'</td>
											...
											endforeach;
											$count = $i;
											$blankLines = ((count($machineList)) - $i);
											if($blankLines > 0):
										...
										$blankLines = (count($machineList) > 0) ? count($machineList) : 1;
										for ($j = 1; $j <= $blankLines; $j++):
											echo '<tr>
												<td>&nbsp;</td>
												<td></td>
