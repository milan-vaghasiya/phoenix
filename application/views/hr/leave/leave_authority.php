                ...
                <form>
                    <div class="row">
						...
						<div class="col-md-12 form-group">
                            <label for="pla_id">HOD Authority</label>
                            <select name="pla_id" id="pla_id" class="form-control select2" multiple>
							</select>
						    <div class="pla_id error"></div>
                        </div>
                        <div class="col-md-12 form-group">
                            <label for="fla_id">HR Authority</label>
...
function openLeaveAuthModal(emp_id,pla_id,fla,emp_name="Leave Authority Modal"){ 
    $("#leaveAuthModal").modal('show');
	$("#leaveAuthModal .modal-title").html(emp_name);
    $('#emp_id').val(emp_id);
    $.ajax({
        url: base_url + controller + '/getEmpLeaveAuthDetail',
        data: {emp_id:emp_id,pla_id:pla_id,fla:fla},
        type: "POST",
        dataType:'json',
        success:function(data){
            $('#pla_id').html(data.plaOptions);
            $('#fla_id').html(data.flaOptions);
            initSelect2();
        }
    });
}
...
function saveAuthority(){
	...
	if(emp_id)
	{
		$.ajax({
			...
			data:{id:emp_id,fla_id:fla_id,pla_id:pla_id},
			type: "POST",
			dataType:"json",
		}).done(function(data){
		    if(data.status===0){
