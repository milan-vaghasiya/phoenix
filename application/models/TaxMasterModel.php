...
class TaxMasterModel extends MasterModel{
	...
	public function getDTRows($data){
        ...
        $data['select'] = 'tax_master.*, party_master.party_name as account_name, (CASE WHEN tax_master.tax_type =1 THEN "Purchase" WHEN tax_master.tax_type =2 THEN "Sales" ELSE "" END) as tax_type_name,
    ...
    public function getTaxList($data){
        ...
        if(!empty($data['tax_ids']))
            $queryData['where_in']['id'] = $data['tax_ids'];
        if(!empty($data['type']))
            $queryData['where']['tax_type'] = $data['type'];
