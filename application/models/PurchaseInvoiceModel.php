...
class PurchaseInvoiceModel extends MasterModel{
    ...
    public function getDTRows($data){
        ...
        $data['select'] = "trans_main.id, trans_main.trans_number, trans_main.trans_date, trans_main.party_name, trans_main.taxable_amount, trans_main.gst_amount, trans_main.net_amount, company_info.company_code";
    ...
    public function save($data){
        try{
            ...
            if($this->checkDuplicate($data) > 0):
                return ['status'=>0,'message'=>['trans_number'=>'Inv. No. is duplicate.']];
            endif;
    ...
    /* public function agreementEffect($data){
        try{
            $this->db->trans_begin();

            $agreementList = $this->agreementModel->getAgreementList($data);

            if(!empty($agreementList)):
                $unsettleQty = floatval($data['qty']);
                foreach($agreementList as $row):
                    $recQty = (floatval($unsettleQty) > floatval($row->pending_qty))?floatval($row->pending_qty):floatval($unsettleQty);

                    if($recQty > 0):
                        $agreementData = [
                            'id' => '',
                            'table_name' => $this->transChild,
                            'description' => "PURINV AGMT ADJ",
                            'main_ref_id' => $data['main_ref_id'],
                            'child_ref_id' => $data['child_ref_id'],
                            'i_col_1' => $row->id,
                            'd_col_1' => $recQty,
                            'date_col_1' => date("Y-m-d")
                        ];
                        $this->store($this->transDetails,$agreementData);

                        $setData = array();
                        $setData['tableName'] = "agreement";
                        $setData['where']['id'] = $row->id;
                        $setData['set']['rec_qty'] = 'rec_qty, + '.floatval($recQty);
                        $this->setValue($setData);
                    endif;

                    $unsettleQty -= $recQty;
                    if($unsettleQty <= 0): break; endif;
                endforeach;
    ...
    public function agreementReverseEffect($data){
        try{
            $this->db->trans_begin();

            $queryData['tableName'] = $this->transDetails;
            $queryData['select'] = "id,i_col_1,d_col_1";
            $queryData['where']['table_name'] = $this->transChild;
            $queryData['where']['description'] = 'PURINV AGMT ADJ';
            $queryData['where']['main_ref_id'] = $data['id'];
            $result = $this->rows($queryData);
            ...
            foreach($result as $row):
                $setData = array();
                $setData['tableName'] = "agreement";
                $setData['where']['id'] = $row->i_col_1;
                $setData['set_value']['rec_qty'] = 'IF(`rec_qty` - '.floatval($row->d_col_1).' >= 0, `rec_qty` - '.floatval($row->d_col_1).', 0)';
                $this->setValue($setData);

                $this->trash($this->transDetails,['id'=>$row->id]);
            endforeach;
