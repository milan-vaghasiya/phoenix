...
class LeaveAuthorityModel extends MasterModel
{
	...
	public function getLeaveAuthority($postData){
		...
		$postData['select'] = "employee_master.id,employee_master.emp_code,employee_master.emp_name,employee_master.fla_id,employee_master.pla_id,";
		$postData['select'] .= '
		(
        ...
        $postData['where']['employee_master.status'] = 1;
    ...
    public function saveAuthority($data){
        try{
            ...
            $result = Array();
            if(!empty($data['id'])):			         
                $result = $this->edit($this->empMaster, ['id'=>$data['id']], ['fla_id' => $data['fla_id'], 'pla_id' => $data['pla_id'], 'updated_by' => $data['updated_by'], 'updated_at' => $data['updated_at']]);
            endif;
