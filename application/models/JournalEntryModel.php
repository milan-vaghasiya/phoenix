...
class JournalEntryModel extends MasterModel{
    ...
    public function getDTRows($data){
        ...
        $data['select'] = 'trans_main.id,trans_main.order_type,trans_ledger.trans_number,trans_ledger.trans_date,party_master.party_name as acc_name,trans_ledger.amount,trans_ledger.remark,trans_ledger.c_or_d';
    ...
    public function saveHavalaEntry($data){
        try{
            $this->db->trans_begin();
            ...
            $data['vou_acc_id'] = $data['itemData'][0]['acc_id'];
            $data['party_id'] = $data['opp_acc_id'];
            $data['net_amount'] = $data['taxable_amount'] = $data['total_amount'] = array_sum(array_column($data['itemData'],'credit_amount')) + (array_sum(array_column($data['itemData'],'debit_amount')) * -1);	
            $data['ledger_eff'] = 1;
            ...
            //remove old trans
            $this->remove($this->transChild,['trans_main_id'=>$result['id']]);
            $this->transMainModel->deleteLedgerTrans($result['id']);
            ...
            foreach($itemData as $row):
                ...
                $transChildData = [
                    ...
                    'item_id' => $row['acc_id'],
                    'price' => $row['price'],
                    'p_or_m' => ($row['cr_dr'] == "DR")?-1:1,
                    'item_remark'=>$row['item_remark'],
                    'is_delete' => 0
                ];
                $this->store($this->transChild,$transChildData);
                ...
                $transLedgerData['vou_acc_id'] = $row['acc_id'];
                $transLedgerData['opp_acc_id'] = $data['opp_acc_id'];
                ...
                $transLedgerData['opp_acc_id'] = $row['acc_id'];
                $transLedgerData['vou_acc_id'] = $data['opp_acc_id'];
                $transLedgerData['c_or_d'] = ($row['cr_dr'] == "DR")?"CR":"DR";
                $this->transMainModel->storeTransLedger($transLedgerData);
            endforeach;
