...
class PurchaseModel extends MasterModel{
	...
	public function getDTRows($data){
        ...
        $data['select'] = "purchase_enquiry.*, IFNULL(pm.party_name,'') as party_name,IFNULL(pq.quote_count,0) as quotation_count,pq.price,pq.qty as quot_qty,item_master.uom as unit_name,pq.enq_id,pq.quote_no,purchase_quotation.quote_remark,purchase_quotation.enq_id,purchase_quotation.id as quote_id,trans_main.trans_number as order_number,(purchase_enquiry.qty - IFNULL(transChild.po_qty,0)) as pending_qty,transChild.po_qty,item_master.item_code,item_master.item_name,purchase_quotation.quote_no,purchase_quotation.quote_date,purchase_quotation.feasible,purchase_quotation.qty as moq,purchase_quotation.price,purchase_quotation.lead_time,purchase_quotation.delivery_date,project_master.project_name,purchase_indent.trans_number as indent_number";
		$data['leftJoin']['party_master pm'] = "pm.id = purchase_enquiry.party_id";
		$data['leftJoin']['project_master'] = "project_master.id = purchase_enquiry.project_id";
		...
		$data['leftJoin']['(SELECT SUM(qty) as po_qty,ref_id,trans_main_id,from_entry_type FROM trans_child WHERE is_delete = 0 GROUP BY ref_id) as transChild'] = "transChild.ref_id = purchase_enquiry.id AND transChild.from_entry_type = 130";
		$data['leftJoin']['trans_main'] = "transChild.trans_main_id = trans_main.id";
		$data['leftJoin']['purchase_indent'] = "purchase_indent.id = purchase_enquiry.req_id";
	...
	public function getPurchaseEnqList($param=[]){
        ...
        $queryData['select'] = "purchase_enquiry.*, IFNULL(pm.party_name,'') as party_name,IFNULL(pq.quote_count,0) as quotation_count,pq.price,pq.qty as quot_qty,item_master.uom as unit_name,pq.enq_id,pq.quote_no,purchase_quotation.quote_remark,item_master.gst_per,pq.quote_date,item_master.hsn_code,purchase_quotation.delivery_date,purchase_quotation.mill_name";
		...
		if(!empty($param['orderData'])){
			$queryData['select'] .= ',trans_main.trans_number as order_number,(purchase_enquiry.qty - IFNULL(transChild.po_qty,0)) as pending_qty,transChild.po_qty'; 
			$queryData['leftJoin']['(SELECT SUM(qty) as po_qty,ref_id,trans_main_id,from_entry_type FROM trans_child WHERE is_delete = 0 GROUP BY ref_id) as transChild'] = "transChild.ref_id = purchase_enquiry.id AND transChild.from_entry_type = 130";
			$queryData['leftJoin']['trans_main'] = "transChild.trans_main_id = trans_main.id ";
		}
		...
		try {
			...
			if(!empty($termsData)):
                foreach($termsData as $row):
                    $row['id'] = "";
                    $row['table_name'] = $this->purchase_enquiry;
                    $row['description'] = "PE TERMS";
                    $row['main_ref_id'] = $result['id'];
                    $row['t_col_1'] = $row['t_col_2'];
                    $row['t_col_2'] = $data['trans_number'];
	...
	public function getQuotationData($param=[]){
		...
		$queryData['select'] = "purchase_quotation.*, IFNULL(pm.party_name,'') as party_name,purchase_enquiry.item_name,purchase_enquiry.trans_number as enq_number,purchase_enquiry.trans_date as enq_date,purchase_enquiry.uom,item_master.item_code";
		...
		if(!empty($param['lastPOPrice'])){
			...
			$queryData['leftJoin']['(SELECT party_id, trans_child.price, trans_child.item_id FROM trans_child JOIN trans_main ON trans_main.id = trans_child.trans_main_id WHERE trans_child.is_delete = 0 AND trans_child.id IN( SELECT MAX(pt.id) FROM trans_child pt JOIN trans_main transMain ON transMain.id = pt.trans_main_id WHERE pt.is_delete = 0 GROUP BY transMain.party_id, pt.item_id)) as purchaseOrder'] = "purchaseOrder.party_id = purchase_quotation.party_id AND purchaseOrder.item_id = purchase_quotation.item_id "; //04-10-2024
		}
		...
		if(!empty($param['enq_number'])){ $queryData['where']['purchase_enquiry.trans_number'] = $param['enq_number']; } 
		if(!empty($param['group_by'])){ $queryData['group_by'][] = $param['group_by']; }
		if(!empty($param['enq_status'])){ $queryData['where']['purchase_enquiry.trans_status'] = $param['enq_status']; }
	...
	public function chageEnqStatus($data){
		try {
			...
			if($quoteDetail->item_id == -1){
				...
				$itemData = $this->item->getItem(['item_name'=>$quoteDetail->item_name,'item_types'=>1]);
				...
				if(!empty($itemData->id)){
					...
					$itemData = $this->store('item_master', $quoteData);
					$item_id = $itemData['id'];
				}
			}
