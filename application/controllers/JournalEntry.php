...
class JournalEntry extends MY_Controller{
    ...
    private $transJournalForm = "journal_entry/trans_journal_form";
    ...
    public function save(){
        ...
        if(empty($data['trans_no']))
            $errorMessage['trans_number'] = "JV. No. is required.";
    ...
    public function edit($id=0){
        $mainId = (!empty($id))?$id:$this->input->post('id');
        $this->data['dataRow'] = $dataRow = $this->journalEntry->getJournalEntry($mainId);
        if(!empty($id)):
            $this->data['entry_type'] = $this->data['entryData']->id;
            $this->data['ledgerList'] = $this->party->getPartyList();
            $this->load->view($this->transJournalForm, $this->data);
        else:
    ...
    public function addHavalaEntry(){
        $this->data['entry_type'] = $this->data['entryData']->id;
        $this->data['trans_prefix'] = $this->data['entryData']->trans_prefix;
        $this->data['trans_no'] = $this->data['entryData']->trans_no;
        $this->data['trans_number'] = $this->data['trans_prefix'].$this->data['trans_no'];
    ...
    public function saveHavalaEntry(){
        $data = $this->input->post();
        $errorMessage = array();
        ...
        if (!empty($errorMessage)) :
            ...
            $this->printJson($this->journalEntry->saveHavalaEntry($data));
        endif;
    }
}
?>