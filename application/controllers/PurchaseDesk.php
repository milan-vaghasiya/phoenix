...
class PurchaseDesk extends MY_Controller{
	...
	public function addEnqFromIndent($id){ 
        ...
        $this->data['itemList'] = $this->item->getItemList(['item_type'=>1]);
        $this->data['unitData'] = $this->item->itemUnits();
        $this->data['indentItemList'] = $this->purchaseIndent->getPurchaseIndentForOrder($id);
		$this->data['itemList'] = $this->item->getItemList(['item_type'=>1]);
        $this->data['projectList'] = $this->project->getProjectList(['is_active'=>0]); 
        $this->load->view($this->form,$this->data);
	}
	...
	public function printEnquiry($jsonData=""){
		...
		$mpdf->SetWatermarkImage($logo,0.05,array(100,40));
	...
	public function quotationComparison(){
		$this->data['enqList'] = $this->purchase->getQuotationData(['trans_status'=>1, 'enq_status'=>4, 'group_by'=>'purchase_enquiry.trans_number', 'multi_row'=>1]);
        $this->load->view('purchase_desk/compare_form',$this->data);
	}

	public function getCompareList(){
		$data = $this->input->post();
		$itemData = $this->purchase->getQuotationData(['enq_number'=>$data['enq_number'], 'trans_status'=>1, 'enq_status'=>4, 'group_by'=>'purchase_quotation.party_id', 'order_by'=>1, 'multi_row'=>1]);
	...
	public function getPartyComparison(){
		$data = $this->input->post();
		$quotData = $this->purchase->getQuotationData(['party_id'=>$data['party_id'], 'enq_number'=>$data['enq_number'], 'lastPOPrice'=>1, 'trans_status'=>1, 'enq_status'=>4, 'multi_row'=>1]);
		
		$itemData = array_reduce($quotData, function($quotDetails, $row) {
			$item_name = ((!empty($row->item_code) ? '[ '.$row->item_code.' ] ' : '').$row->item_name);
			$quotDetails[$item_name][$row->party_name] = $row;
			return $quotDetails;
		}, []);

		$html='';
		if (!empty($itemData)) {
			foreach ($itemData as $item_name => $partyData) {
				$html .= '<h4 class="fs-15 text-primary border-bottom-sm">'.$item_name.'</h4>';
				$html .= '<table class="table jpExcelTable mb-5">
					<thead>
						<tr class="text-center">
							<th style="width:15%;">Details</th>';
							foreach ($partyData as $party_name => $compareData) {
								$html .= '<th>'.$party_name.'</th>';
							}
						$html .= '</tr>
					</thead>
					<tbody>
						<tr class="text-center">
							<th>Enq No/Date</th>';
							foreach ($partyData as $party_name => $row) {
