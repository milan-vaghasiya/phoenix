...
class GateInward extends MY_Controller{
	...
	public function getPoNumberList($param=[]){
        ...
        if (!empty($poList)) {
            foreach ($poList as $row) {
                $selected = (!empty($data['po_id']) && $data['po_id'] == $row->po_id) ? "selected" : "";
                $disabled = (!empty($data['po_id']) && $data['po_id'] != $row->po_id) ? "disabled" : "";
                $options .= '<option value="'.$row->po_id.'" data-po_no="'.$row->trans_number.'" '.$selected.' '.$disabled.'>'.$row->trans_number.'</option>';
            }
        }
    ...
    public function getItemList(){
        ...
        if(empty($data['po_id'])):
            ...
            foreach($itemList as $row):
                $options .= '<option value="'.$row->item_id.'" data-po_trans_id="'.$row->po_trans_id.'" data-price="'.$row->price.'" data-disc_per="'.$row->disc_per.'" >[ '.$row->item_code.' ] '.$row->item_name.' [ Pending Qty : '.(!empty($row->pending_qty) ? floatval($row->pending_qty) : 0).' ]</option>'; 
            endforeach;
        endif;
    ...
    public function getMirNextNo(){
        $data = $this->input->post();
        $nextNo = $this->transMainModel->getMirNextNo(2);
	...
	public function edit(){
        $data = $this->input->post();
        $this->data['grnData'] = $grnData = $this->gateInward->getGateInwardList(['id'=>$data['grn_id'],'result_type'=>1]);
        $this->data['grnTransData'] = $grnTransData = $this->gateInward->getInwardItem(['id'=>$data['id']]);
        $this->data['partyList'] = $this->party->getPartyList(['party_category'=>[2,3]]);
        $this->data['itemList'] = $this->item->getItemList(['item_type'=>[1,2,3]]);
        $this->data['projectList'] = $this->project->getProjectList(['is_active'=>0]);
        $this->data['poData'] = $this->getPoNumberList(['party_id'=>$grnData->party_id, 'project_id'=>$grnData->project_id, 'po_id'=>$grnTransData->po_id]);
    ...
    public function delete(){
        ...
        if(empty($data['id'])):
            ...
            $this->printJson($this->gateInward->deleteGrn($data));
        endif;
    }
	...
	public function updateGRN(){
		...
		if(empty($data['itemData']['po_id']))
            $errorMessage['po_id'] = "Purchase Order is required.";
