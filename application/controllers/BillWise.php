...
class BillWise extends MY_Controller{
    ...
    public function getUnsettledTransactions(){
        ...
        foreach($result as $row):
            $billWiseParam = "{'postData':{'id' : ".$row->id.",'party_id':".$row->party_id.",'c_or_d':'".(($row->c_or_d == "CR")?"DR":"CR")."','cm_id':".$row->cm_id."},'modal_id' : 'bs-right-lg-modal', 'form_id' : 'billWiseForm', 'title' : 'Bill Wise Settlement','call_function':'loadReferenceTransactions','js_store_fn':'customStore'}";
            ...
            $tbody .= '<tr>
                ...
                <td class="text-right">'.moneyFormatIndia(floatval($row->net_amount)).'</td>
                <td class="text-right">'.moneyFormatIndia(floatval($row->net_amount - $row->pending_amount)).'</td>
                <td class="text-right">'.moneyFormatIndia(floatval($row->pending_amount)).'</td>
                <td class="text-right">'.moneyFormatIndia(floatval($balance)).'</td>
    ...
    public function getSettledTransaction(){
        ...
        foreach($settledTrans as $row):
            ...
            $tbody .= '<tr>
                ...
                <td>'.moneyFormatIndia(floatval($row->net_amount)).'</td>
                <td>'.moneyFormatIndia(floatval($row->amount)).'</td>
