...
$(document).ready(function(){
	...
	$(document).on("change",'#order_type',function(){
		...
		if(order_type == "Increase Purchase"){
		...
		if(($.inArray($("#saveCreditNote #port_code").val(),["GExp","GInc"]) >= 0)){ 
			$('#itemForm #stock_eff').val("0");
			$('#tempItem .itemStockEff').val("0"); 
		}
		...
		if(order_type == "Sales Return"){
			trans_prefix = trans_prefix.replace('C', 'S');
		}else{
			trans_prefix = trans_prefix.replace('S', 'C');
		}
		...
		if(selected_cm_id == cm_id && selected_order_type == order_type){
			...
			$.ajax({
				url : base_url + controller + '/getNextCreditNoteNo',
				type : 'post',
	...
	$('#doc_no').typeahead({
		source: function(query, result){
			$.ajax({
				...
				success:function(data){
					...
					$("#saveCreditNote #port_code").val("");
					$("#saveCreditNote #port_code").attr('data-controller','salesInvoice');
		...
		updater: function(item) {
			...
			$("#saveCreditNote #port_code").val((($.inArray(item.vou_name_s,["GExp","GInc"]) >= 0)?item.vou_name_s:""));
			if(item.vou_name_s == "GExp"){ $("#saveCreditNote #port_code").attr('data-controller','gstExpense'); }
			else if(item.vou_name_s == "GInc"){ $("#saveCreditNote #port_code").attr('data-controller','gstIncome'); }
			else{ $("#saveCreditNote #port_code").attr('data-controller','salesInvoice'); }
...
function initItemForm(){
	if($("#saveCreditNote #port_code").val() == ""){
...
function createInvoice(){	
	var port_code = $("#saveCreditNote #port_code").val();	
	$("#creditNoteItems tbody").html("");
