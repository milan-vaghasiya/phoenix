$(document).ready(function(){
	...
	$(document).on('change',"#generateEwayBill #transaction_type", function() {
		...
		$("#generateEwayBill #from_village_name").val("");
		...
		$("#generateEwayBill #ship_village_name").val("");
		...
		if (transaction_type == "") {
			...
			/* if (party_id == "") {
				...
				$.ajax({
					...
					type: 'post',
					data: {
						inv_id:inv_id,party_id: party_id,transaction_type:transaction_type,document_type:document_type
					},
					dataType: 'json',
					success: function(data) {
						...
						$("#generateEwayBill #from_village_name").val(data.from_village_name);
						...
						//$("#generateEwayBill #from_state").select2();
						$("#generateEwayBill #ship_village_name").val(data.ship_village_name);
...
function ebillFrom(data){
	...
	$.ajax({ 
		...
		//data.modal_id = "bs-right-xl-modal";
